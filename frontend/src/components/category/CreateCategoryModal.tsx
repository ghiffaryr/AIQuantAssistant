...
export default function CreateCategoryModal({
  ...
  const categorySellerUpdate = useCategorySellerCreate({
    successSideEffect: () => {
    ...
    onError: error => {
      errorHandler({
        ...
        axiosErrorHandlerFn: err => {
          setErrorCreateCategory(err);
          setShowCreateCategoryToast(true);
        },
        generalErrorHandlerFn: err => {
          setErrorCreateCategory({ message: err.message });
          setShowCreateCategoryToast(true);
        },
      });
    },
  ...
  const handleSubmitCreateCategory = (e: any) => {
        ...
        <ToastContainer className="position-fixed p-3 top-0 end-0">
          {Object.keys(errorCreateCategory).length > 0 ? (
            ...
            >
              ...
              <Toast.Body>{(errorCreateCategory as any).message}</Toast.Body>
            </Toast>
          ) : (
            <Toast
              onClose={() => setShowCreateCategoryToast(false)}
