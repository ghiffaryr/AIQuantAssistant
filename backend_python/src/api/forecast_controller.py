...
class Predict:
    async def on_post(self, req, resp) -> None:
        ...
        def get_result(params):
            ...
            fs = ForecastServiceImpl()
            fs.retrieve_dataset(stock_code=params[Param.STOCK_CODE])
            fs.impute_dataset(method=params[Param.IMPUTE_METHOD])
            fs.configure_training_parameter(training_window=params[Param.TRAINING_WINDOW], 
                                            model_choice=params[Param.MODEL_CHOICE])
            fs.train()
            prediction = fs.predict(params[Param.FORECASTING_HORIZON])
            return prediction
